...
font_path = 'LP-Bold.ttf'
letters = 'กขคฆงจฉชฌญฎฐฑฒณดตถทธนบปผฝพฟภมยรลวศษสหฬอฮ'
number = 9
size = (165,55)
...
address = os.path.abspath('.') + "/data/"

def gene_text():
    source_l = list(letters)
    source_n = []
    source_blank = [' ', '']
...
def gene_code(n):
    if not os.path.exists(address):
        os.makedirs(address)
    for i in range(n):
        f = open(address + str(i) + '.txt', 'w', encoding='utf8')
        ...
        image = image.rotate(20 * (random.random()-0.5))
